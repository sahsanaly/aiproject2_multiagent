    ...
    def run(self):
        ...
        for i in range(len(self.agents)):
            ...
            if ("registerInitialState" in dir(agent)):
                ...
                if self.catchExceptions:
                    try:
                        #TODO: migrate to timeout.TimeoutFunction (half way but not tested)
                        timed_func = timeout.TimeoutFunction(
                            agent.registerInitialState, int(self.rules.getMaxStartupTime(i)))
                        try:
                            start_time = time.time()
                            _, time_elapsed = timed_func(self.state.deepCopy())
                            time_taken = time.time() - start_time
                            self.totalAgentTimes[i] += time_taken
                            # print(time_elapsed, time_taken)
        ...
        while not self.gameOver:
            ...
            if 'observationFunction' in dir(agent):
                ...
                if self.catchExceptions:
                    try:
                        timed_func = timeout.TimeoutFunction(agent.observationFunction, int(
                            self.rules.getMoveTimeout(agentIndex)))
                        try:
                            start_time = time.time()
                            observation, _ = timed_func(self.state.deepCopy())
                        except timeout.TimeoutFunctionException:
                            skip_action = True
            ...
            if self.catchExceptions:
                try:
                    timed_func = timeout.TimeoutFunction(agent.getAction, int(
                        self.rules.getMoveTimeout(agentIndex)) - int(move_time))
                    try:
                        start_time = time.time()
                        if skip_action:
                            raise timeout.TimeoutFunctionException()
                        action, move_time2 = timed_func(observation)
                        move_time += time.time() - start_time
                        #TODO: which move time do we care? 
                        # I think it should be move_time2 and remove the rest
                    except timeout.TimeoutFunctionException:
