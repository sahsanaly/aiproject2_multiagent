...
def run(lay, layName, pac, ghosts, disp, nGames=1, name='games', timeout=10):
    """
    ...
    games = pacman.runGames(lay, pac, ghosts, disp,
                            nGames, False, catchExceptions=True, timeout=timeout)
...
class PacmanGameTreeTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.timeout = int(self.testDict.get('timeouts', 10))
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        stats = run(lay, self.layout_name, pac, [DirectionalGhost(
            i + 1) for i in range(2)], disp, name=self.alg, timeout=self.timeout)
        if stats['timeouts'] > 0:
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        random.seed(self.seed)
        print(f"Playing {self.numGames} games with seed {self.seed} and timeouts set to {self.maxTime} seconds.")
